...
/**
  ******************************************************************************
  * @file           : main.c
  * @brief          : Main program body for coordinated sensor transmission (4 sensors)
  ******************************************************************************
  * @attention
  *
...
/* Private define ------------------------------------------------------------*/
#define BASE_DELAY_MS 10  // Base delay (in ms) for staggered transmission
...
// Set a unique sensorID for each sensor.
// For Sensor 1: 0x01, Sensor 2: 0x02, Sensor 3: 0x03, Sensor 4: 0x04.
...
void processADCValues(void)
{
    ...
    V2 = 0;
    for (l = Ng + Ns + r; l < Ng + 2 * Ns + r; l++) {
        V2 += adcVal[l];
    }
    SLO = (V1 - V2) / 16;
    
    V3 = 0;
    for (j = 3 * Ng + 2 * Ns + r; j < 3 * Ng + 3 * Ns + r; j++) {
        V3 += adcVal[j];
    }

    V4 = 0;
    for (m = 3 * Ng + 3 * Ns + 2 * r; m < 3 * Ng + 4 * Ns + 2 * r; m++) {
        V4 += adcVal[m];
    }
    SRO = (V4 - V3) / 16;
